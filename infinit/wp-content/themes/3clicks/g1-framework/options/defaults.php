    ...
    class Redux_Options {
        ...
        */
        function __construct($sections = array(), $args = array(), $extra_tabs = array()) {    
            ...
            $defaults['sections_filter_name'] = ''; // Must be defined by theme/plugin
            $defaults['skin_options'] = array();
            ...
            // Get sections
            $this->sections = apply_filters('redux-opts-sections-' . $this->args['sections_filter_name'], $sections);

            // Get extra tabs
            $this->extra_tabs = apply_filters('redux-opts-extra-tabs-' . $this->args['opt_name'], $extra_tabs);
        
            // Set option with defaults
            add_action('wp_loaded', array(&$this, '_set_default_options'), 10000);
            ...
            add_action('do_feed_reduxopts-' . $this->args['opt_name'], array(&$this, '_download_options'), 1, 2);
        ...
        function _options_page() {
            if($this->args['page_type'] == 'submenu') {
                ...
                if(true === $this->args['allow_sub_menu']) {
                    ...
                    add_theme_page($this->args['page_title'], '', $this->args['page_cap'], $this->args['page_slug'], create_function('$a', "return null;"));
        ...
        */
         ...
         * @since Redux_Options 1.0.0
        */
        function _download_options( $arg , $get_request = true ) {
            //-'.$this->args['opt_name']
            if($get_request && (!isset($_GET['secret']) || $_GET['secret'] != md5(AUTH_KEY.SECURE_AUTH_KEY))) {
                wp_die('Invalid Secret for options use');
                exit;
            }

            if($get_request && !isset($_GET['feed'])){
                wp_die('No Feed Defined');
                exit;
            }

            $opt_name = $get_request ? $_GET['feed'] : $this->args['opt_name'];
            $opt_name = str_replace('reduxopts-', '', $opt_name);

            $backup_options = get_option($opt_name);
            $backup_options['redux-opts-backup'] = '1';

            if ($get_request && !empty($_GET['type']) && $_GET['type'] === 'skin' ) {
                foreach ( $backup_options as $backup_option_name => $backup_option_value ) {
                    if ( !in_array($backup_option_name, $this->args['skin_options']) ) {
                        unset($backup_options[$backup_option_name]);
                    }
                }
            }
        ...
        */
        function _validate_options($plugin_options) {
            ...
            if(!empty($plugin_options['import'])) {
                ...
                $import_skin = !empty($_POST['import_type']) && $_POST['import_type'] === 'skin';

                if ( $import_skin ) {
                    $all_options = $this->_download_options(null, false);
                    $skin_options = unserialize(trim($import,'###'));

                    $imported_options = array_merge($all_options, $skin_options);
                } else {
        ...
        */
        function _options_page_html() {
            ...
            if(true === $this->args['dev_mode']) {
				...
				if(isset($this->args['icon_type']) && $this->args['icon_type'] == 'image') {
            ...
            echo '<div id="redux-opts-main-loaded"><span>'. __('Loading...', Redux_TEXT_DOMAIN) .'</span></div>';
            ...
            if(true === $this->args['show_import_export']) {
                ...
                echo '<p>';
                    echo '<a href="javascript:void(0);" id="redux-opts-import-code-button" class="button-secondary">' . __('Import from file', Redux_TEXT_DOMAIN) . '</a>';
                    echo ' <a href="javascript:void(0);" id="redux-opts-import-link-button" class="button-secondary">' . __('Import from URL', Redux_TEXT_DOMAIN) . '</a>';
                    echo ' <a href="javascript:void(0);" id="redux-opts-import-skin-button" class="button-secondary">' . __('Import Skin', Redux_TEXT_DOMAIN) . '</a>';
                echo '</p>';
                ...
                echo '<p><a href="javascript:void(0);" id="redux-opts-export-code-copy" class="button-secondary">' . __('Copy', Redux_TEXT_DOMAIN) . '</a>';
                echo ' <a href="'.add_query_arg(array('feed' => 'reduxopts-'.$this->args['opt_name'], 'action' => 'download_options', 'secret' => md5(AUTH_KEY.SECURE_AUTH_KEY)), site_url()).'" id="redux-opts-export-code-dl" class="button-primary">Download All Options</a>';
                echo ' <a href="'.add_query_arg(array('feed' => 'reduxopts-'.$this->args['opt_name'], 'action' => 'download_options', 'type' => 'skin', 'secret' => md5(AUTH_KEY.SECURE_AUTH_KEY)), site_url()).'" id="redux-skin-export-code-dl" class="button-primary">Download Skin</a>';
                echo ' <a href="javascript:void(0);" id="redux-opts-export-link" class="button-secondary">' . __('Copy Link', Redux_TEXT_DOMAIN) . '</a></p>';
